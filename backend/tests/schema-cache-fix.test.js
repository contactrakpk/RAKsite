...
test('schema-safe ordering, cache busting, and loader recovery are configured', async () => {
  ...
  assert.match(await read('assets/css/styles.css'), /\.category-page-products \.product-card \.card-image,[\s\S]*?\.trending-row \.product-card \.card-image,[\s\S]*?height: 180px !important;[\s\S]*?background-color: #f8fafc;/);
...
test('featured products are persisted and rendered in category sliders', async () => {
  ...
  assert.match(stylesCss, /\.category-products-slider \.product-card \.card-image \{[\s\S]*?height: 180px !important/);
