...
const sanitizeDatabaseImage = (str) => {
  ...
  let cleaned = str.replace(/:[0-9]+$/, '').trim();
  if (cleaned.startsWith('http://') || cleaned.startsWith('https://')) return cleaned;
  const jpegMatch = cleaned.match(/\/9j\/[A-Za-z0-9+/=]+/);
  const pngMatch = cleaned.match(/iVBORw0[A-Za-z0-9+/=]+/);
  if (jpegMatch) return `data:image/jpeg;base64,${jpegMatch[0]}`;
...
window.cleanBase64Image = sanitizeDatabaseImage;
